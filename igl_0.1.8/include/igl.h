...
#if PLATFORM_WIN32
    /* pragma-based linkage */
#   pragma comment(lib, "opengl32.lib")
#   pragma comment(lib, "glu32.lib")
#   pragma comment(lib, "IGL.lib")

    /* not defined on Win32 systems */
#   define M_PI 3.1415926536f
#   ifndef PI
#       define PI 3.1415926536f
#   endif

#   define sleep(_a)            Sleep (_a*1000)
#   define SWAPBUFFERS(_ctx)    SwapBuffers ((_ctx)->hdc)
#   define SHOWCURSOR()         ShowCursor (TRUE);
#   define HIDECURSOR()         ShowCursor (FALSE);
    /* this is done to prevent unclean exits */
#   undef exit
#   define exit(_ret) PostQuitMessage(_ret)
...
/* following two lines are here for compatibility with IGL < 0.1.7 */
#   define IGL_MSG_BEGIN    IGL_MSG_PROCESS
#   define IGL_MSG_END
#else
extern int
igl_X11init ();
...
#   define SWAPBUFFERS(_ctx)    glXSwapBuffers (igl->appInstance, (_ctx)->wnd)
//#   define SHOWCURSOR()         XDefineCursor (igl->appInstance, IGL_CTX ()->wnd, XC_arrow)
//#   define HIDECURSOR()         XUndefineCursor (igl->appInstance, IGL_CTX ()->wnd)
#   define SHOWCURSOR()
#   define HIDECURSOR()
#endif
...
/*
    note on IRIS_XXX (system/graphic) defines:
    if you change a define, you have to clean compile
    the panel library and ep to make it work. IRIS_GT
    version of ep enables an additional panel for controlling
    color (hue, saturation, opacity). don't define more than
    one, unless you want to see problems...

    the best choice is IRIS_4D, as it produces no flicker.
    IRIS_NEWS doesn't work (mouse clicks are lost), IRIS_GT
    produces lots of flicker and has resize problems with the
    first panel on X11 and on Win32 the labels are invisible.
    so by default, IRIS_4D is enabled. if you want to experiment
    or find/resolve any bugs that are hanging around, please do.
*/
#define IRIS_4D
//#define IRIS_GT
//#define IRIS_NEWS
...
#if PLATFORM_WIN32  /* this is a system call on UN*X */
void
